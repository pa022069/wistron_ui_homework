...
const Default = (props: DefaultProps): JSX.Element => {
  ...
  return (
    <div className="inputStyle__container">
      <div
        className={`inputStyle__default ${
          isFocus ? "inputStyle__default--focus" : ""
        }`}
      >
        <label
          className="font__normal fontSize__sm"
          onClick={handleClick}
          htmlFor={props.name}
        >
          {props.title}
      ...
      <div className="inputStyle__container">
        ...
        >
          ...
          <input
            ...
            name={props.name}
            id={props.id}
            onChange={(element) => checkPasswordFormat(element.target.value)}
          />
        </div>
        <BsEye
          onClick={() => setIsShowValue((prev: boolean) => !prev)}
          className={`inputStyle__icon ${
            isShowValue ? "inputStyle__icon--show" : ""
          }`}
        />
      </div>
      {inputValue && (
        <div className="inputStyle__validated">
          <div
            className={`${isMatchLength ? "inputStyle__validated--check" : ""}`}
          >
            <BsCheckCircleFill className="inputStyle__validated--icon" />
            <p className="inputStyle__validated--text">8 Characters min.</p>
          </div>
          <div
            className={`${isMatchNumber ? "inputStyle__validated--check" : ""}`}
          >
            <BsCheckCircleFill className="inputStyle__validated--icon" />
            <p className="inputStyle__validated--text">One number</p>
          </div>
        </div>
      )}
    </>
